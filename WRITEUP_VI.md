...
**Dự án:** [GitHub Repo](https://github.com/) | [Video Demo](https://youtube.com/)
...
Hệ thống sử dụng `Workflow` của Google ADK (với fallback `SequentialAgent` để tương thích ADK 2.x) để điều phối bốn sub-agent chuyên biệt:
...
**Mood Analysis Agent** nhận ghi chú đã làm sạch và thực hiện phân loại cảm xúc bằng Gemini 2.5 Flash. Nó xuất ra một `mood_report` có cấu trúc chứa nhãn tâm trạng chính (ví dụ: `happy`, `stressed`, `melancholic`), điểm cường độ và giải thích. Đầu ra có cấu trúc này là thứ điều khiển minh hoạ chibi — không phải văn bản thô.
...
|---|---|---|
| `test_orchestrator.py` | 32 | Pipeline E2E, đầu ra agent, lưu trữ SQLite |
...
| *(tích hợp)* | 7 | ADK tools của Memory Agent: trend, recap, streak |
...
| Khái niệm | Vị trí |
|---|---|
| ✅ Hệ thống Multi-agent (ADK) | `app/orchestrator.py` — pipeline 4-agent SequentialAgent/Workflow |
| ✅ MCP Server | `mcp_server/chibi_mcp_server.py` — FastMCP stdio, Imagen 3 |
| ✅ Tính năng Bảo mật | `app/agents/capture_agent.py` — InputSanitizer + SECURITY_AUDIT_LOG |
