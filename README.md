...
### 🎨 Chibi Art Generated by Imagen 3 MCP

Real chibi illustrations produced by the `chibi_illustrator_agent` calling the Imagen 3 MCP server — each image is generated live from the user's mood analysis output.

| Melancholic (Score: 0.3) | Comforted (Score: 0.7) |
|:---:|:---:|
| ![Chibi sad rainy day](docs/screenshots/chibi_art_sad_rainy_day.png) | ![Chibi cozy hug](docs/screenshots/chibi_art_cozy_hug.png) |
| *"Rainy day, sitting by the window"* | *"Warm hug, feeling safe"* |

> 💡 Images are saved to `output/chibi_images/` with a timestamped filename and UUID suffix (e.g. `chibi_20260625_060448_85ef28df.png`).

### 🖥️ ADK Dev UI — Pipeline in Action

The full multi-agent pipeline is visible and inspectable via the ADK Web UI (`uv run adk web` → `http://localhost:8000`):

![ADK Dev UI pipeline](docs/screenshots/adk_dev_ui_pipeline.png)
